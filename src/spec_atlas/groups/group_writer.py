                ...
                try:
                        ...
                        try:
                            ...
                            if spec_session:
                                related_specs = (
                                    ...
                                    .filter(
                                        Spec.repo == repo_id,
                                        Spec.component_ref.in_(
                                            group.member_spec_refs or []
                                        ),
                                    )
                                    .all()
                                )
                            ...
                            logger.warning(f"Failed to generate summary for group {group.path}: {e}")
                            # Use fallback summary
                            group.summary_md = _generate_fallback_summary(
                                group, member_nodes
                            )
                    ...
                    if spec_session:
                        _link_specs_to_group(
                            group, member_nodes, spec_session, analysis_session
                        )
...
def _link_specs_to_group(
    ...
    try:
        ...
        logger.debug(
            f"Linked {len(specs)} specs to group {group.path}"
        )
